...
interface SignatureProps extends StylesProps<typeof signatureStyles> {
  className?: string
  initial?: boolean
  whileInView?: boolean
...
const pathVariants: Variants = {
  ...
  visible: (transition: Transition) => ({
    ...
    transition: {
      ...
      duration: 4.5,
      ...transition,
      opacity: { duration: 0.01, ...transition },
    },
  }),
}
...
function Signature(props: SignatureProps) {
  ...
  const progressPathOffset = useTransform(activeProgress, [0, 1], [1, 0])
  const progressPathLength = useTransform(activeProgress, [0, 1], [0, 1])
  ...
  return (
    ...
    >
      <motion.path
        ...
        custom={transition}
        initial={progress ? false : initial ? 'hidden' : false}
        animate={
          progress
            ? undefined
            : whileInView
...
export { Signature, signatureStyles, type SignatureProps }
